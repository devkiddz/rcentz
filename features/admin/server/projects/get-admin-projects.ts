...
export const getAdminProjects = cache(async () => {
  ...
  const projects = rawProjects.map(project => {
    const completed = project.milestones.filter(milestone => {
      return milestone.status === 'COMPLETED';
    }).length;

    const active = project.milestones.filter(milestone => {
      return (
    ...
    const remaining = project.milestones.filter(milestone => {
  ...
  const summary = {
    ...
    onHold: projects.filter(project => {
      return project.status === 'ON_HOLD';
    }).length,

    unassigned: projects.filter(project => {
...
export type AdminProjectsData = Awaited<
  ReturnType<typeof getAdminProjects>
>;
