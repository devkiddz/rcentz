...
export function BusinessManagementGrowth() {
  const webDevelopmentT = useTranslations('WebDevelopmentStory');

  const servicesHeroT = useTranslations('ServicesHero');

  const reduceMotion = useReducedMotion();

  function getSignalLabel(id: (typeof serviceSignals)[number]['id']) {
    if (id === 'webApplications') {
      return webDevelopmentT('capabilities.customWebApplications');
    }

    if (id === 'businessSystems') {
      return servicesHeroT('slides.businessSystems');
    }
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                {serviceSignals.map((item, index) => (
                  <motion.div
                    ...
                    <span className="mt-2 block text-[8px] leading-3 text-muted">
                      {getSignalLabel(item.id)}
                    </span>
