      ...
      <div className="group relative aspect-[16/10] overflow-hidden rounded-xl border border-border bg-background">
        {activeScreenshot ? (
          <img
            ...
            alt={activeScreenshot.alt ?? activeScreenshot.caption ?? `${projectName} project screenshot`}
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-background">
            <div className="flex max-w-xs flex-col items-center px-6 text-center">
              ...
              <p className="mt-4 text-sm font-semibold text-foreground">Project preview</p>
        ...
        <div className="absolute left-4 top-4">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/45 px-3 py-1.5 text-white backdrop-blur-md">
            ...
            <span className="text-[10px] font-semibold uppercase tracking-[0.12em]">Rcentz Project</span>
          </div>
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 px-5 pb-7 sm:px-6 sm:pb-8">
          <div className="max-w-lg">
            <h3 className="text-xl font-semibold tracking-[-0.04em] text-white sm:text-2xl">{projectName}</h3>

            <p className="mt-2 max-w-md text-xs leading-5 text-white/70 sm:text-sm">{projectTagline}</p>
          </div>
        </div>
      </div>
