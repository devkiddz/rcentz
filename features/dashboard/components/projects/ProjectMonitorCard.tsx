...
import type { ProjectMonitorData, ProjectMonitorHealthStatus } from './project-monitor.types';

type ProjectMonitorCardProps = {
  project: ProjectMonitorData;
};
...
function getHealthPresentation(status: ProjectMonitorHealthStatus) {
  switch (status) {
    case 'AT_RISK':
      return {
        icon: AlertTriangle,
...
function ProjectScreenshot({ project }: { project: ProjectMonitorData }) {
        ...
        <div
          ...
          <div className="min-w-0">
            ...
            {project.screenshot.caption ? (
              <p className="mt-1 truncate text-[9px] text-white/70">{project.screenshot.caption}</p>
            ) : null}
          </div>
...
function OnlineHealthSignal() {
  ...
  if (reduceMotion) {
    return <span aria-hidden="true" className="size-2 rounded-full bg-theme-accent" />;
  }
...
function ProjectHealthSignal({ status }: { status: ProjectMonitorHealthStatus }) {
  if (status === 'ON_TRACK') {
  ...
  return (
    <span
      aria-hidden="true"
      className={['size-2', 'rounded-full', status === 'AT_RISK' ? 'bg-rose-500' : 'bg-amber-500'].join(' ')}
    />
  );
}

function ProjectHealthCard({ project }: ProjectMonitorCardProps) {
  const healthPresentation = getHealthPresentation(project.health.status);

  const HealthIcon = healthPresentation.icon;

  return (
    <div className={['rounded-2xl', 'border', 'p-4', healthPresentation.surfaceClassName].join(' ')}>
      <div className="flex items-center gap-2">
        <HealthIcon aria-hidden="true" className={['size-4', healthPresentation.labelClassName].join(' ')} />
      ...
      <div className="mt-3 flex items-center gap-2.5">
        ...
        <p className={['text-sm', 'font-semibold', healthPresentation.labelClassName].join(' ')}>
...
function DeliverySignalsCard({ project }: ProjectMonitorCardProps) {
  return (
...
function OverallProgressCard({ project }: ProjectMonitorCardProps) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface-raised p-4">
      <div className="flex items-end justify-between gap-3">
        <div>
          <p
            className={[
              ...
              'font-semibold',
              'tracking-[-0.06em]',
              progressPresentation.textClassName
            ].join(' ')}>
            {overallProgress}%
          </p>
        ...
        <Activity aria-hidden="true" className={['size-5', progressPresentation.textClassName].join(' ')} />
      </div>
      ...
      <div className="mt-4 h-2 overflow-hidden rounded-full bg-surface-muted">
        <div
          className={[
            ...
            'transition-[width]',
            'duration-500',
            progressPresentation.barClassName
          ].join(' ')}
          style={{
...
function ProjectSnapshot({ project }: ProjectMonitorCardProps) {
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-surface-raised">
      <div className="grid grid-cols-2 sm:grid-cols-4">
        <div className="border-b border-r border-border p-3 sm:border-b-0">
          <div className="flex items-center gap-1.5">
            <FileText aria-hidden="true" className="size-3 text-theme-accent" />
        ...
        <div className="border-b border-border p-3 sm:border-b-0 sm:border-r">
          <div className="flex items-center gap-1.5">
            <Flag aria-hidden="true" className="size-3 text-theme-accent" />
        ...
        <div className="border-r border-border p-3">
          <div className="flex items-center gap-1.5">
            <ListTodo aria-hidden="true" className="size-3 text-theme-accent" />
        ...
        <div className="p-3">
          <div className="flex items-center gap-1.5">
            <AlertTriangle aria-hidden="true" className="size-3 text-theme-accent" />
...
function NextMilestoneCard({ project }: ProjectMonitorCardProps) {
  return (
    <div className="rounded-2xl border border-border bg-surface-raised p-4">
      <div className="flex items-center gap-2">
...
function PageHealthCard({ project }: ProjectMonitorCardProps) {
  return (
    <div className="rounded-2xl border border-border bg-surface-raised p-4">
...
function ProjectMilestoneProgress({ project }: ProjectMonitorCardProps) {
  return (
    <div className="rounded-2xl border border-border bg-surface-raised p-4">
              ...
              <div key={milestone.id} className="space-y-2.5">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex min-w-0 items-center gap-2">
                    <span
                      className={[
                        ...
                        'shrink-0',
                        'rounded-full',
                        progressPresentation.dotClassName
                      ].join(' ')}
                    />
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <span
                      className={['text-[10px]', 'font-semibold', progressPresentation.textClassName].join(
                        ' '
                      )}>
                      {milestoneProgress}%
                    </span>
...
export function ProjectMonitorCard({ project }: ProjectMonitorCardProps) {
  return (
